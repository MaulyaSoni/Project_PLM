    ...
    <div className="animate-fade-in text-white pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-4xl font-display font-bold tracking-tight text-white mb-2">Bills of Materials</h1>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-card border border-white/10 shadow-xl p-6">
          ...
          <div className="relative z-10 flex flex-col h-full justify-center">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display font-bold text-lg text-white">Immutable Operations</h3>
            </div>
            <p className="text-white/90 leading-relaxed font-medium text-[15px]">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-card border border-white/10 shadow-xl p-6">
          ...
          <div className="relative z-10 flex flex-col h-full justify-center">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display font-bold text-lg text-white">Controlled Revisions</h3>
            </div>
            <p className="text-white/90 leading-relaxed font-medium text-[15px]">
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap p-4 rounded-2xl bg-card/30 backdrop-blur-md border border-white/5 shadow-lg">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <Input placeholder="Search by product..." value={search} onChange={e => setSearch(e.target.value)} className="pl-11 h-12 bg-black/20 border-white/10 text-white placeholder:text-white/50 focus-visible:ring-primary/50 focus-visible:border-primary/50 rounded-xl" />
        </div>
        <div className="flex bg-black/20 border border-white/10 rounded-xl p-1 h-12">
          {(['ALL', 'ACTIVE', 'ARCHIVED'] as const).map(s => (
            <button key={s} onClick={() => setStatusFilter(s)} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${statusFilter === s ? 'bg-primary/20 text-primary shadow-[0_0_10px_rgba(0,242,255,0.2)]' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>{s === 'ALL' ? 'All' : s.charAt(0) + s.slice(1).toLowerCase()}</button>
          ))}
        </div>
      </div>
            ...
            <Table>
              <TableHeader className="bg-white/[0.02]">
                <TableRow className="border-white/5 hover:bg-transparent">
                  <TableHead className="text-white font-semibold py-5 px-6 text-xs uppercase tracking-wider">Product</TableHead>
                  <TableHead className="text-white font-semibold py-5 text-xs uppercase tracking-wider">Version</TableHead>
                  <TableHead className="text-white font-semibold py-5 text-xs uppercase tracking-wider">Components</TableHead>
                  <TableHead className="text-white font-semibold py-5 text-xs uppercase tracking-wider">Operations</TableHead>
              ...
              <TableBody>
                {filtered.map(b => (
                  <TableRow key={b.id} className="border-white/5 transition-colors hover:bg-white/[0.04] group">
                    <TableCell className="font-medium text-white px-6 py-4 text-base">{b.productName}</TableCell>
                    <TableCell className="text-white/80 font-mono">v{b.currentVersion}</TableCell>
                    <TableCell className="text-white font-mono">{b.components.length}</TableCell>
                    <TableCell className="text-white font-mono">{b.operations.length}</TableCell>
                    <TableCell><StatusBadge status={b.status} /></TableCell>
                    <TableCell className="text-white/70 text-sm font-mono">{b.createdAt}</TableCell>
                    <TableCell className="text-right pr-6 space-x-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:text-white hover:bg-white/10 rounded-lg transition-all" onClick={() => setDetailBOM(b)}><Eye className="h-4 w-4" /></Button>
                      {canEdit && <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:text-white hover:bg-white/10 rounded-lg transition-all disabled:opacity-30" disabled={b.status === 'ARCHIVED'} onClick={() => handleRaiseEco(b)}><GitPullRequest className="h-4 w-4" /></Button>}
                      {canArchive && b.status === 'ACTIVE' && <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all" onClick={() => setArchiveTarget(b.id)}><Archive className="h-4 w-4" /></Button>}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
